...
const MONTHS_IN_YEAR = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun',
...
const formatDate = dateObj => {
  ...
  let dayOfWeek = DAYS_IN_WEEK[dateObj.getDay()];
  let year = dateObj.getFullYear();
  let month = MONTHS_IN_YEAR[dateObj.getMonth()];

  return `${dayOfWeek} ${month} ${dayOfMonth} ${year}`;
}


const mailCount = emailData => {
  let emails = emailData.split('##||##');
  let dateStrings = emails.flatMap(email => email.match(/\d{2}-\d{2}-\d{4}/g));
  let realDates = dateStrings.map(date => new Date(date));
  let earliestDate = new Date(Math.min(...realDates));
  let latestDate = new Date(Math.max(...realDates));
